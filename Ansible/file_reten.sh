...
- name: Database Connectivity and Schema Group Build Play
  connection: local
  vars:
    ira_client_name: "{{ inventory_hostname.split('-') }}"
    common_region: "{{ lookup('env', 'Customer_Environment') }}_{{ ira_client_name[0] }}"
    Customer_Environment: "{{ common_region }}"
    job_data_center: "{{ lookup('env', 'JOB_NAME').split('_')[0].split('/')[-1] }}"
    ansible_python_interpreter: 'python'
    ansible_async_dir: "{{ common_job_workspace }}/.ansible_async"
  block:  
    ...
    - name: Display App DB export job parameters
      debug:
        msg: |
         common region is {{ common_region }}
          job_data_center is {{ job_data_center }}
          inventory_hostname is {{ inventory_hostname }}
          ira_clients is {{ ira_clients | default('') }}
          customer_environment is "{{ lookup('env', 'Customer_Environment') }}"

    - name: Set Data Center database host
      set_fact:
        db_oracle_server_host: "{{ lookup('vars', 'db_oracle_server_host_' + job_data_center|lower) }}"
    
    - name: Display DB oracle parameters
      debug:
        msg: |       
         db_oracle_server_host is {{ db_oracle_server_host }}
         db_oracle_server_port is {{ db_oracle_server_port }}
         db_service_name is {{ db_service_name }}
         db_schema_list is {{ db_schema_list }}
         IRA db oracle server host is {{ ira_db_oracle_server_host }}

    - name: Block to verify database connectivity
      block:
        - name: Verify Database connectivity
          shell:
            cmd: |
              export ORACLE_HOME={{ db_oracle_client_home }};
              export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:{{ db_oracle_client_home }}/lib;
              {{ db_oracle_sqlplus }} "{{ _conn }}"
            stdin: "{{ _sql_query }}"
          vars:
            _conn: "{{ db_export_user_id }}/{{ db_export_passwd }}@//{{ db_oracle_server_host }}:{{ db_oracle_server_port }}/{{ db_service_name }}"
            _sql_query: |
              set heading off
              EXIT;
          register: verify_db_connection
      ...
      always:
        ...
        - name: Write out database connectivity to log
          copy:
            dest: "{{ common_job_workspace }}/{{ app_name }}-{{ _client }}-DB-Connectivity-Test-Output-{{ _timestamp }}.log"
            content: |
              Command:
              {{ verify_db_connection.cmd }}
              ...
              {{ verify_db_connection.stderr }}
          vars:
            _timestamp: "{{ '%Y-%m-%d-%H%M%S' | strftime(ansible_date_time.epoch) }}"
            _client: "{{ inventory_hostname.split('-')[0]| upper }}"


    - name: Set Application Region Schemas
      set_fact:
        common_region_schemas: "{{ common_region_schemas | default([]) + [ (item|dict2items)[0].key ] }}"
      loop: "{{ db_schema_list }}"
      loop_control:
        label: "{{ item }}"
      
    - name: Display Application Region Schemas
      debug:
        msg: |
          common_region_schemas are {{ common_region_schemas }}

    - name: Set Target schemas
      set_fact:
        target_schemas: >-
          {{ target_schema.split(',') if target_schema is defined and target_schema|length > 0
              else common_region_schemas
          }}

    - name: Show Target Schemas for job
      debug:
        msg: |
          Target schemas are: {{ target_schemas }}
      run_once: true

    - name: Perform Schema Export Play
      block: 
        - name: Perform DB Schema Export
          shell:
            cmd: |
              export ORACLE_HOME={{ db_oracle_client_home }};
              export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:{{ db_oracle_client_home }}/lib;
              {{ db_oracle_expdp }} "{{ _conn }}" SCHEMAS={{ item }} DIRECTORY=DUMPDIR DUMPFILE={{ _dumpfile }} LOGFILE={{ _logfile }} CLUSTER={{ db_export_cluster }} PARALLEL={{ db_export_parallel_processes }} COMPRESSION={{ db_export_compression }}
          vars:
            _conn: "{{ db_export_user_id }}/{{ db_export_passwd }}@//{{ db_oracle_server_host }}:{{ db_oracle_server_port }}/{{ db_service_name }}"
            _dumpfile: "{{ db_export_group }}_{{ item }}_schema_Jenkins_{{ _timestamp }}_%U.dmp"
            _logfile: "{{ db_export_group }}_{{ item }}_schema_Jenkins_{{ _timestamp }}.log"
            _timestamp: "{{ '%Y-%m-%d-%H%M%S' | strftime(ansible_date_time.epoch) }}"
          throttle: "{{ db_max_parallel_schema_exports | default(2) }}"
          async: "{{ db_export_runtime_max|int }}"
          poll: 30
          loop: "{{ target_schemas }}"
          loop_control: 
            label: "Schema: {{ item }}"
          register: db_schema_export_results
        
      always:
        - name: Display database schema export results
          debug:
            msg: |
              Start: {{ ira_db_schema.start }}
              End: {{ ira_db_schema.end }}
              Result Code: {{ ira_db_schema.rc }}
              Log file: {{ app_name }}-DB-Schema-{{ ira_db_schema.item }}-Export-{{ _start_timestamp }}.log"
             
              _________________________________
          vars:
            _start_timestamp: "{{ (ira_db_schema.start[:19]|to_datetime).strftime('%Y-%m-%d-%H%M%S') }}"
          loop: "{{ db_schema_export_results.results }}"
          loop_control:
            label: "DB Schema: {{ ira_db_schema.item }}"
            loop_var: ira_db_schema
          when:
            - ira_db_schema.rc == 0

        - name: Write out database schema export results to log file
          copy:
            dest: "{{ common_job_workspace }}/{{ app_name }}-DB-Schema-{{ ira_db_schema_log.item }}-Export-{{ _start_timestamp }}.log"
            content: |
              Start & End: {{ ira_db_schema_log.start }} - {{ ira_db_schema_log.end }}
              Result Code: {{ ira_db_schema_log.rc }}
          ...
          vars:
           _start_timestamp: "{{ (ira_db_schema_log.start[:19]|to_datetime).strftime('%Y-%m-%d-%H%M%S') }}"
          throttle: 1
          loop: "{{ db_schema_export_results.results }}"
          loop_control:
            label: "DB Schema Log: {{ ira_db_schema_log.item }}"
            loop_var: ira_db_schema_log
          when:
