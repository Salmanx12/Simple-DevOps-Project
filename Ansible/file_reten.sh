...
# Combine sunmapper logs into one list
- name: Set Sunmapper log list
  set_fact:
    sunmapper_log_paths: "{{ list_of_sunmapper_logs.results | map(attribute='files') | sum(start=[]) | map(attribute='path') | list }}"
...
- name: Display list of sunmapper log files that will be zipped (lvl 1)
  debug:
    msg: "{{ _list_sm }}"
    verbosity: 1
  vars:
    _list_sm: "{{ sunmapper_log_paths }}"

- name: Display list of ira log files that will be zipped (lvl 1)
  debug:
    msg: "{{ _list_ira }}"
    verbosity: 1
  vars:
    _list_ira: "{{ ira_log_paths }}"    
...
- name: "Create gz archive of the previous {{ ira_log_dump_days }} day(s) of IRA logs"
  archive:
    path: "{{ ira_log_paths }}"
    dest: "{{ automation_log_dumps_dir.dest }}/{{ _gzip_ira_file }}"
    owner: "{{ ira_app_user }}"
    group: "{{ ira_app_group }}"
    excludes:
     - "*.gz"
    ...
    format: gz
  vars:
    _list_ira: "{{ list_of_logs | list }}"
    _gzip_ira_file: "{{ ansible_date_time.date }}-{{ (ansible_date_time.time).replace(':','') }}-{{ inventory_hostname|upper }}-logs.gz"
  register: ira_log_gzip_results

- name: "Create gz archive of the previous {{ ira_log_dump_days }} day(s) of SUnmapper logs"
  archive:
    path: "{{ sunmapper_log_paths }}"
    dest: "{{ automation_log_dumps_dir.dest }}/{{ _gzip_sm_file }}"
    owner: "{{ ira_app_user }}"
    group: "{{ ira_app_group }}"
    excludes:
     - "*.gz"
    ...
    format: gz
  vars:
    _list_sm: "{{ list_of_logs | list }}"
    _gzip_sm_file: "{{ ansible_date_time.date }}-{{ (ansible_date_time.time).replace(':','') }}-{{ inventory_hostname|upper }}-logs.gz"
  register: sm_log_gzip_results  

- name: Fetch IRA gzip file from hosts and place in job workspace
  fetch:
    src: "{{ ira_log_gzip_results.dest }}"
    dest: "{{ ira_job_workspace }}/"
    flat: true
  register: fetch_results_ira
  notify:
    - log dump cleanup

- name: Fetch Sunmapper Gzip file from hosts and place in job workspace
  fetch:
    src: "{{ sm_log_gzip_results.dest }}"
    dest: "{{ ira_job_workspace }}/"
    flat: true
  register: fetch_results_sm
  notify:
    - log dump cleanup   
